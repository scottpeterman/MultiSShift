...
from multisshift.dialogs.edit_session_dialog import EditSessionDialog
from multisshift.dialogs.add_session_dialog import AddSessionDialog
from multisshift.ui.context_menus import SessionContextMenu, FolderContextMenu, RootContextMenu
from multisshift.ui.credentials_dialog import CredentialsDialog
...
class SessionManager(QTreeWidget):
    ...
    def on_item_double_click(self, item):
        if hasattr(item, 'is_leaf_item') and item.is_leaf_item:
            ...
            if dialog.exec() == QtWidgets.QDialog.DialogCode.Accepted:
                ...
                self.main_window.add_new_ssh_tab(refBinding=item.refBinding, username=self.username, auth_type = "password", auth=self.password)
    ...
    def load_sessions_from_yaml(self, yaml_file="./sessions/sessions.yaml"):
        try:
            ...
            # self.notify("YAML Error", f"YAML parsing error: {e}")
            raise e # or some other failure indicator
